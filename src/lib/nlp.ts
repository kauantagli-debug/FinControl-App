...
export function parseMessage(text: string, categories: Category[]): ParsedMessage {
    ...
    if (!categoryId) {
        const keywords: Record<string, string[]> = {
            'alimentação': ['comida', 'lanche', 'restaurante', 'ifood', 'mercado', 'pizza', 'almoço', 'janta'],
            'transporte': ['uber', '99', 'taxi', 'bus', 'onibus', 'gasolina', 'posto', 'estacionamento'],
            'salário': ['salario', 'pagamento', 'freela', 'freelance'],
            'lazer': ['cinema', 'jogo', 'steam', 'netflix', 'spotify'],
